...
export function getChatHistoryPaginationKey(
  ...
  if (pageIndex === 0)
    return `/api/history?limit=${PAGE_SIZE}&is_clean_chat=${isCleanChat}`;

  const firstChatFromPage = previousPageData.chats.at(-1);

  if (!firstChatFromPage) return null;

  return `/api/history?ending_before=${firstChatFromPage.id}&limit=${PAGE_SIZE}&is_clean_chat=${isCleanChat}`;
}
  ...
  if (hasEmptyChatHistory) {
    return (
      <SidebarGroup>
        <SidebarGroupContent>
          <div className="flex flex-row gap-2 justify-center items-center px-2 w-full text-sm text-zinc-500">
            Здесь появятся диалоги, как только начнете переписку!
          </div>
        </SidebarGroupContent>
      </SidebarGroup>
    );
        ...
        <SidebarGroupContent>
          ...
          {hasReachedEnd ? (
            <div className="flex flex-row gap-2 justify-center items-center px-2 mt-8 w-full text-sm text-zinc-500">
              Вы дошли до конца истории чатов.
            </div>
          ) : (
      ...
      <AlertDialog open={showDeleteDialog} onOpenChange={setShowDeleteDialog}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Действие нельзя отменить. Чат будет удалён без возможности
              восстановления.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
